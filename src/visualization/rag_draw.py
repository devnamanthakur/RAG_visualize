...
def draw_rag(processes, allocated_matrix, need_matrix, total_resources):
    ...
    proc_pos = {p: (2.0, y_vals_proc[i]) for i, p in enumerate(processes)}
    ...
    res_pos = dict(zip(res_names, [(8.0, y) for y in y_vals_res]))
    ...
    ax.set_title(
        "Resource Allocation Graph", fontsize=14, fontweight="bold"
    )
